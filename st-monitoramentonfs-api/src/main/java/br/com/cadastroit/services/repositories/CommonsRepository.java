...
public class CommonsRepository {
    ...
    public <T> T findById(Long id, Class<T> clazz, EntityManagerFactory entityManagerFactory) {
        ...
        try {
            ...
            throw new ConstrNfsException(String.format(DbLayerMessage.NO_RESULT_POR_ID.message(), getTblName(clazz), "", id));
        } finally {
            em.clear();
            em.close();
        }
    ...
    // String OBJECT = getTblName(from.getModel().getJavaType());
    public static String getTblName(Class<?> clazz) {
        return clazz.getAnnotation(Table.class).name();
    }

    public BigDecimal convertEnumToBigDecimal(StatusProcessamento stProc) {
        return BigDecimal.valueOf(stProc.getKeys());
    }

    public void addToPredicates(CriteriaBuilder cb, List<Predicate> predicates, In<BigDecimal> inDmStProcValues,
                                       BigDecimal dmStProc) {
        if (inDmStProcValues != null) {
            inDmStProcValues.value(dmStProc);
            predicates.add(cb.and(inDmStProcValues));
        }
    }
}
