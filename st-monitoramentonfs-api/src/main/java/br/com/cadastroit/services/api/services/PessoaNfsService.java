...
public class PessoaNfsService {

	private static final String MODE = "Error on %s mode to %s, [error] = %s";
	private static final String OBJECT = "PESSOA_NFS";
	private static final String ORDER = "order";
	
	private PessoaNfsRepositoryImpl repository;


	public Long maxId(EntityManagerFactory entityManagerFactory) throws PessoaNfsException {
		...
		try {
			...
			throw new PessoaNfsException(String.format(MODE, "maxId", OBJECT, ex.getMessage()));
		} finally {
			em.clear();
			em.close();
		}
	...
	public List<Predicate> createPredicates(Long nfServId, PessoaNfsDto entityDto, Root<PessoaNfs> from,
		...
		if (entityDto != null && entityDto.getCodPart() != null) {
			predicates.add(cb.equal(from.get(PessoaNfs_.codPart), entityDto.getCodPart()));
		}
