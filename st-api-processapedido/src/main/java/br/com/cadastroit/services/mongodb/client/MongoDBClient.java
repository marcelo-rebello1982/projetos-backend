...
public class MongoDBClient {
	...
	public MongoTemplate mongoTemplate() {
		...
		if (OsDetect.OS_NAME().contains("windows"))
			HOST_FILE = "C:\\workspace\\st-mongo-config\\nosql-processarelatorio.properties";
		String host = "";
		String database = "";
		Integer port = 27017;
		String username = "";
		...
		MongoClientSettings settings = MongoClientSettings.builder().applyToSslSettings(builder -> {
			...
			builder.maxSize(60)
					...
					.minSize(20)
					.build();
		}).applyConnectionString(connectionString).uuidRepresentation(UuidRepresentation.STANDARD).applyToConnectionPoolSettings(pool -> {
