...
public class DesifPlanoContaException extends PersistenceException {

	private static final long serialVersionUID = 8521813705756906479L;

	public DesifPlanoContaException(String message) {
		super(message);
	}

	public DesifPlanoContaException(String message, Throwable cause) {
		super(buildMessage(message, cause), cause);
		message = "[Generic Internal errors] = ";
		throw new DesifPlanoContaException(message);
	}
	...
	public DesifPlanoContaException(Throwable cause) {		
		super(cause);
		String message = "";
		if(cause.getCause() != null && cause.getCause().getClass().equals(PersistenceException.class)) {
			...
			if(e.getCause() != null && e.getCause().getClass().equals(ConstraintViolationException.class)) {
				...
				if(c.getCause() != null && c.getCause().getClass().equals(SQLIntegrityConstraintViolationException.class)) {
					...
					message += ". [Internal errors] = "+c.getMessage()+", Constraint Name = "+c.getConstraintName()+", SQL = "+c.getMessage();
				}
			} else {
				message += ". [Generic Internal errors] = "+e.getMessage();
			}
