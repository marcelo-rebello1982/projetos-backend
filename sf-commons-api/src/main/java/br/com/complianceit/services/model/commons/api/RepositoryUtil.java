package br.com.complianceit.services.model.commons.api;
...
import br.com.complianceit.enterprise.entity.persistence.domain.MultOrg;
import br.com.complianceit.enterprise.entity.persistence.domain.MultOrg_;
import br.com.complianceit.enterprise.entity.persistence.domain.Usuario;
import br.com.complianceit.enterprise.entity.persistence.domain.UsuarioEmpresa;
import br.com.complianceit.enterprise.entity.persistence.domain.UsuarioEmpresa_;
import br.com.complianceit.enterprise.entity.persistence.domain.Usuario_;
import br.com.complianceit.services.model.commons.annotations.FilterIndex;

public class RepositoryUtil {

	protected Object converterField(FilterIndex filterIndex, Object value) {
		if(!filterIndex.converter().isInstance(Object.class)) {
			if(filterIndex.converter().equals(BigDecimal.class)) {
	...
	protected List<Long> retrieveConsolida(Long idEmpresa, Credential credential, EntityManager entityManager) {
        ...
        TypedQuery<UsuarioEmpresa> tQuery = entityManager.createQuery(cqConsolida.select(fromConsolida).where(cbConsolida.and(cbConsolida.equal(joinUEU.get(Usuario_.id), credential.getUsuarioId()),
	...
	protected Multimap<String, List<Long>> retrieveConsolida(Long idEmpresa, String CnpjEmpTomadora, Credential credential, EntityManager entityManager) {
	    ...
	    TypedQuery<UsuarioEmpresa> tQuery = entityManager.createQuery(cqConsolida.select(fromConsolida)
	    					.where(
	    						cbConsolida.and(cbConsolida.equal(joinUEU.get(Usuario_.id), credential.getUsuarioId()),
	    							cbConsolida.equal(joinUMO.get(MultOrg_.cd), credential.getCodigo()))));
	    			if(!tQuery.getResultList().isEmpty()) {
	    			List<UsuarioEmpresa> ueCollection = tQuery.getResultList();
	    				ueCollection.forEach(ue->{
